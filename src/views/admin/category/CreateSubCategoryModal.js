...
const CreateSubCategoryModal = props => {
    ...
    const handleAddingSubCategory = () => {
        ...
        axios.post("/categories/sub-categories",
            data,
            {
                headers: {
                    Authorization: token,
            ...
            .then(response => {
                ...
                setStatus({
                    type: 'SUCCESS',
                    message: "Value is Added in Sub-Category List",
                })
                setSubCategory("")
                setImage(null)
                setPreview(null)
    ...
    return (
        <Modal open={open} onClose={onClose} blockScroll={false}>
            <div style={{ width: 500 }}>
                <p className="pageHeading mt-4">Enter Sub-Category Name: </p>
                <input type="text" className="createFromInputField mt-4" onChange={e => setSubCategory(e.target.value)} value={subCategory} />
                {preview ? (
...

                {
                    loading ? (
                        ...
                        <button className="button button-primary mt-4 w-20" style={{ marginLeft: 0 }} onClick={handleAddingSubCategory}>Add</button>
                    )
                }
