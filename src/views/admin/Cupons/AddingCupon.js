...
const AddingCupon = props => {
    ...
    React.useEffect(() => {
        if(user?.permissions)
        {
            if(!(permission(user.permissions, 'offers_coupon', 'create')) && (user.user_type_id != 1))
                history.push('/admin');
        }
    }, [user])
    ...
    const submitCouponData = async (event) => {
        event.preventDefault();
          ...
          try {
                ...
                const response =  await axios.post('coupons',data, { headers: {
                        Authorization: token,
                ...
                <div className="w-2/3">
                    ...
                    <div className="mt-8 flex justify-end">
                        {(loading) ? (
                            ...
                            <>
                                <button onClick={submitCouponData} className="button button-primary w-32">Add Cupon</button>
                            </>
                        )}
